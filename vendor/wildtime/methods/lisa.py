...
def lisa(
    ...
    X2 = torch.zeros(X1.shape).to(device)
    Y2 = torch.zeros(Y1.shape, dtype=torch.long).to(device)
    ...
    for eachclass in class_ids:
        ...
        sel_x2, sel_y2 = dataset.get_lisa_new_sample(
            time_idx, eachclass.item(), sel_idx.shape[0]
        )
